...
connector (see its docstring). Same schema, same bounded tool-calling loop,
same conversational ask_user flow as agent_github.py; the only thing that
changed is the data source and the domain. No identity scoping here on
purpose (any logged-in staff member sees all tickets) — this demo isn't
re-proving the access-control story agent_github.py and agent_customer.py
already cover.

RELIABILITY LAYER: ported from agent_gmail.py / agent_slack.py 2026-08-25.
Before this port, this was the least-hardened connector in the codebase —
no fabrication guardrail at all, no lazy-clarify-question detection, and a
bare "if not tool_calls" branch that only ever nudged. The same proven
recovery/guardrail stack those two connectors already validated against
live failures (truncated JSON, blanket-quoted apostrophes, whole tool calls
written as plain text instead of real function calls, and the model
inventing placeholder content like "Status" or "Ticket 1" instead of using
real fetched data) is copied over wholesale here, adapted to the
get_tickets/ticket-fields domain. This is no longer meant to "stay minimal"
in the sense of skipping that layer — it's now hardened to match the other
real connectors; the only thing that stays deliberately different is that
this connector never falls back to a plain conversational `{"text": ...}`
reply (see the system prompt and the `if not tool_calls:` branch below) —
that was already this file's intent before the port, and the port doesn't
change it.
"""
...
def _scrub_surrogates(s):
    ...
    renders. Ported verbatim from agent_gmail.py — see that file's copy for the full story
...
def _repair_candidates(s):
    ...
    backslash-escaped double quote embedded in real content — _close_truncated_json() assumes a
...
TOOL_NAMES = {"get_tickets", "ask_user", "render_view"}
...
def _parse_tool_calls_from_text(reply, verbose=False):
    ...
    of three shapes:
      ...
      3. Python-call syntax, MULTIPLE calls chained one per line — e.g. writing
         get_tickets(...) and render_view(...) back to back in the same reply, as if narrating a
         plan instead of calling either for real.
...
def _find_placeholder_labels(view):
    ...
    placeholders exploit, since each one is a single word. Adapted from agent_gmail.py's copy
    (which was scoped to Gmail's "Email N"/"Subject"/"From" placeholders) for this connector's
    domain — support tickets, whose fields are subject/description, status, priority, and
    category rather than Gmail's sender/subject/snippet."""
    bad = []
...
NO_TOOL_CALL_NUDGE = (
    ...
    "Do not write JSON in your reply — call an actual tool (get_tickets, ask_user, or "
...
def _original_user_request(messages):
    ...
    agent_gmail.py's / agent_healthcare.py's copy of this function."""
    for m in messages:
        if m.get("role") == "user":
...
def _is_lazy_clarifying_question(question, user_request):
    """Same heuristic as agent_gmail.py / agent_healthcare.py — see those files' copies for the
    full reasoning. High word overlap with the original request and almost no new vocabulary
    means the question is just echoing the request back instead of actually narrowing it."""
    def tokens(s):
    ...
    for step in range(max_steps):
        ...
        try:
            try:
                resp = client.chat.completions.create(
                    ...
                    # Was 700, raised to 3500 (2026-08-25 port) to match agent_gmail.py's /
                    # agent_slack.py's tuned value — the same "components" truncation failure
                    # mode those connectors hit at lower budgets applies here too, since it's the
                    # same model generating the same shape of JSON.
                    max_tokens=3500,
                )
            except Exception:
        ...
        if not tool_calls:
            ...
            if recovered_calls:
                ...
                # a reply chains get_tickets then render_view: the fetch
                # runs first, so fetched_data is already True by the time
                # the render_view entry below is checked, exactly mirroring
                # what would have happened with two real, separate tool
                # calls.
                fetch_note = None
                ...
                for call_name, call_args in recovered_calls:
                    ...
                    elif call_name == "get_tickets":
                        if fetched_data:
                            if verbose:
                                print(f"  [step {step+1}] recovered get_tickets, but data was already fetched — skipping the redundant call", file=sys.stderr)
                            continue
                        try:
                            result = get_tickets(**call_args)
                            fetched_data = True
                            if verbose:
                                print(f"  [step {step+1}] recovered get_tickets({call_args}) — ran it for real", file=sys.stderr)
                            fetch_note = "Here is the real result of that call — use these exact values, then call render_view: " + json.dumps(result)
                        except Exception as e:
                            if verbose:
                                print(f"  [step {step+1}] recovered get_tickets raised: {e}", file=sys.stderr)
            ...
            # Unlike agent_gmail.py / agent_slack.py, this connector never
            # accepts a plain conversational `{"text": ...}` reply — the
            # system prompt tells the model it must always call render_view
            # before finishing, and that design intent predates this port
            # (agent_github.py, this file's original structural template,
            # shares it too). So when nothing could be recovered as a real
            # tool call, every non-tool-call reply just gets nudged, the
            # same as the pre-port behavior — it's never treated as a
