"""
Composition Engine, connected to a REAL data source instead of the mock
connector — a live GitHub repository's issue tracker. Same schema, same
bounded tool-calling loop, same guardrails as agent_nvidia.py; only the
connector changed, from fabricated incidents to actual GitHub issues.

This is the proof that the architecture doesn't care what's behind the
connector: swap connectors.py for connectors_github.py, point the agent at
one real tool instead of two mock ones, and the rest of the loop (schema,
validation, retry guardrails, renderer) needs no changes at all.

RELIABILITY LAYER: ported 2026-08-25 from agent_gmail.py (by way of
agent_slack.py, a second independent confirmation of the same port for a
different connector) — this file previously had an older, less-hardened
version of the tool-calling loop that predated a full day of live
debugging against llama-3.1-8b-instruct's tool-calling output on the other
two connectors: truncated JSON, blanket-quoted apostrophes, whole tool
calls written as plain text instead of real function calls, and the model
inventing placeholder content instead of using real fetched data. Rather
than let this connector re-discover each of those bugs on its own the way
Gmail did, the whole proven recovery/guardrail stack was copied over
wholesale — structural JSON repair, plain-text tool-call recovery, the
fabrication guardrail (already partially wired in here) plus a
GitHub-issues-specific placeholder-label denylist, and diagnostic logging
on both rejection and acceptance. The identity-scoping story this
connector already had (`user`/`scope_issues`, see below) is untouched by
this port — it was already correct and is preserved exactly, including
being applied to issues fetched via the newly-added recovered-text path.
"""
...
client = OpenAI(
    ...
    # Without an explicit timeout, the SDK's default is ~10 minutes PER
    # ATTEMPT, and it retries twice on top of that — so a single stalled
    # connection can silently hang for 30+ minutes with zero output, which
    # looks exactly like "the model is thinking" but is actually the network
    # never getting a response. 45s is generous for this model; if a call
    # hasn't come back by then it's not going to.
    timeout=45.0,
    # max_retries=0 is deliberate, not an oversight: the SDK's own retry
    # logic gives every failed call ANOTHER full 45s attempt internally,
    # which then compounds with our own two retry layers below (the
    # tool_choice "required"->"auto" fallback per step, and run_agent()'s
    # one whole-request retry) — three layers of doubling turns a single
    # slow call into a multi-minute wait (measured: ~181s to fail on one
    # step, ~600s+ for a whole failed request). We already retry better
    # than the SDK does (a different tool_choice, then a fresh request),
    # so its internal retry is pure redundant latency, not resilience.
    max_retries=0,
)
...
# 2026-08-25: no model change here — this date's work was porting the reliability layer proven
# on agent_gmail.py/agent_slack.py onto this same model, not re-running the model search.
...
def _scrub_surrogates(s):
    ...
    renders. Ported verbatim from agent_gmail.py (via agent_slack.py), where this was found live
    2026-08-25 — see agent_gmail.py's copy for the full story."""
...
def _unescape_literal_unicode_escapes(s):
    """Fixes a failure mode found live on agent_gmail.py 2026-08-25 (round 7) and ported here: a
...
TOOL_NAMES = {"get_github_issues", "ask_user", "render_view"}
...
def _parse_tool_calls_from_text(reply, verbose=False):
    ...
    of three shapes:
      ...
      3. Python-call syntax, MULTIPLE calls chained one per line — e.g. writing
         get_github_issues(...) and render_view(...) back to back in the same reply, as if
...
def _find_placeholder_labels(view):
    ...
    placeholders exploit, since each one is a single word. Adapted from agent_gmail.py's/
    agent_slack.py's copy of this function to GitHub issues' own field-naming conventions: a
    render_view row here typically shows an issue's title/number/labels/state/author, so the
    denylist and pattern target those instead of Gmail's subject/snippet/from or Slack's
    sender/message-N."""
    bad = []
...
NO_TOOL_CALL_NUDGE = (
    ...
    "Do not write JSON in your reply — call an actual tool (get_github_issues, ask_user, or "
    "render_view) using function calling. Call get_github_issues now."
)
...
def _original_user_request(messages):
    ...
    agent_gmail.py's/agent_slack.py's copy of this function."""
    for m in messages:
        if m.get("role") == "user":
...
def _is_lazy_clarifying_question(question, user_request):
    """Same heuristic as agent_gmail.py/agent_slack.py — see those files' copies for the
    full reasoning. High word overlap with the original request and almost no new vocabulary
    means the question is just echoing the request back instead of actually narrowing it — e.g.
    asked "what issues do you want to see?" in response to "what needs attention in this repo?"
    is lazy; "do you mean open issues, or issues that are open but waiting on review?" is a real
...
def _ask_user_already_used(messages):
    """
    The system prompt tells the model to ask at most one clarifying question,
    but that's a request, not a guarantee — a model can and sometimes will
    ignore it (observed in practice after switching models: it asked the
    same question twice in a row instead of proceeding on the second
    answer). This makes the one-question cap a hard rule enforced in code
    instead of something we just hope the model honors.
    """
    for m in messages:
...
def run_agent(user_request=None, user=None, max_steps=8, verbose=True, messages=None, fetched_data=False):
    """
    Thin retry wrapper around _run_agent_once(). If the whole attempt fails
    on a network error (a timeout or dropped connection talking to NVIDIA's
    API — not a validation failure, not hitting max_steps), it's worth one
    full retry from scratch before surfacing an error: a flaky connection
    that failed on step 2 has a real chance of succeeding cleanly on a
    fresh attempt. Anything else (schema validation failures, running out
    of steps) is a real outcome, not a network blip, so it's returned as-is
    with no retry.

    Two ways to call this:
    - Starting a fresh request: pass user_request and user. messages should
      be left as None.
    - Resuming after a clarifying question: pass user (still required, for
      identity scoping), plus messages and fetched_data taken verbatim from
      a previous {"clarify": ...} result, with the person's answer already
      appended to messages as a {"role": "user", ...} turn. user_request is
      ignored in this case (the real "request" is now the whole messages
      list).

    Returns one of:
      {"render": <valid UI_SCHEMA object>}              - done, ready to show
      {"clarify": question, "messages": [...], "fetched_data": bool}
                                                          - paused, needs an
                                                            answer; resume by
                                                            calling run_agent
                                                            again with those
      ...
      {"error": <text>}                                  - failed
    """
    result = _run_agent_once(user_request, user, max_steps, verbose, messages, fetched_data)
...
def _run_agent_once(user_request, user, max_steps, verbose, messages, fetched_data):
    if messages is None:
        messages = [
        ...
        messages = list(messages)  # don't mutate the caller's list — a retry needs the original

    # Tracks the most recent error string each tool raised, so a
    # deterministic failure (bad .env config, an unreachable GitHub API) can
    # be told apart from a transient one, instead of burning all max_steps
    # retrying something that can never succeed — see agent_slack.py for the
    # live bug this was written to fix.
    tool_failures = {}
    ...
    for step in range(max_steps):
        ...
        try:
            try:
                resp = client.chat.completions.create(
                    ...
                    # Raised from 700 to 3500 (2026-08-25 port) to match the value
                    # agent_gmail.py/agent_slack.py were actually tuned against — the same
                    # "components" truncation failure mode those connectors hit at lower
                    # budgets applies here too, since it's the same model generating the same
                    # shape of JSON. See agent_gmail.py's comment on this same call for the
                    # fuller history of how that number was arrived at.
                    max_tokens=3500,
                )
            except Exception:
                # Either this model rejected tool_choice="required", or the
                # first attempt timed out — either way, one retry with
                # "auto" before giving up on this step.
                resp = client.chat.completions.create(
                    model=MODEL,
                    messages=messages,
                    tools=TOOLS,
        ...
        if not tool_calls:
            ...
            if recovered_calls:
                ...
                # a reply chains get_github_issues then render_view: the
                ...
                for call_name, call_args in recovered_calls:
                    ...
                    elif call_name == "get_github_issues":
                        if fetched_data:
                            if verbose:
                                print(f"  [step {step+1}] recovered get_github_issues, but data was already fetched — skipping the redundant call", file=sys.stderr)
                            continue
                        try:
                            result = get_github_issues(**call_args)
                            before = len(result)
                            result = scope_issues(result, user)
                            if verbose and len(result) != before:
                                print(f"  [step {step+1}] identity scoping: {before} fetched -> {len(result)} authorized for {user['username']}", file=sys.stderr)
                            fetched_data = True
                            if verbose:
                                print(f"  [step {step+1}] recovered get_github_issues({call_args}) — ran it for real", file=sys.stderr)
                            fetch_note = "Here is the real result of that call — use these exact values, then call render_view: " + json.dumps(result)
                        except Exception as e:
                            if verbose:
                                print(f"  [step {step+1}] recovered get_github_issues raised: {e}", file=sys.stderr)
        ...
        for tc in tool_calls:
            ...
            if name == "ask_user":
                # Check history BEFORE this turn's own ask_user call (which
                # is already appended above as part of the assistant
                # message) — so "already used" means a PRIOR step asked,
                # not this one.
                if _ask_user_already_used(messages[:-1]):
                    if verbose:
                ...
                # The tool call needs a matching tool result before the
                # conversation can continue (the API requires it) — this is
                # a placeholder; the person's real answer arrives as a fresh
                # user turn when the caller resumes the conversation.
            ...
            if name == "render_view":
                ...
                # Same content-verification guardrail agent_healthcare.py /
                # agent_gmail.py / agent_slack.py use — see guardrails.py —
                # plus the connector-local placeholder-label denylist
                # (_find_placeholder_labels) layered on top of it.
            ...
            try:
                ...
                ok = bool(fn)
                if ok and name == "get_github_issues":
                    before = len(result)
                    result = scope_issues(result, user)
                    if verbose and len(result) != before:
                        print(f"  [step {step+1}] identity scoping: {before} fetched -> {len(result)} authorized for {user['username']}", file=sys.stderr)
                ...
                if fn and tool_failures.get(name) == err_str:
                    ...
                    # an unreachable/misconfigured GitHub repo), not a
                    # transient one a retry could fix. Stop burning steps on
                    # it and surface the real problem immediately.
                    if verbose:
...
if __name__ == "__main__":
    ...
    while "clarify" in result:
        ...
        result = run_agent(user=user, messages=messages, fetched_data=result.get("fetched_data", False))
